    ...
    <div className="template-v4">
      ...
      <main>
        ...
        <section className="livestream-section">
          ...
          <article className="event-card reveal reveal-up delay-2">
            ...
            <div style={{ fontFamily: "var(--v4-font-sub)", fontSize: 12, letterSpacing: "0.1em", color: "var(--v4-blue-pale)", marginBottom: "1.5rem", lineHeight: 2 }}>{data.event.day}, {data.event.displayDate}<br />Pukul {data.event.livestream.time}</div>
      ...
      {isInvitationOpen && (
        <button className={`music-fab${isPlaying ? " playing" : ""}`} onClick={() => setIsPlaying(!isPlaying)}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#A3C9ED" strokeWidth="1.5"><path d="M9 18V5l12-2v13" /><circle cx="6" cy="18" r="3" /><circle cx="18" cy="16" r="3" /></svg>
        </button>
      )}
      {lightboxImage && (
