...
function PaymentCard({ bank, holderName, hasChip, logo, isQris, image, isAddress, address }: PaymentCardProps) {
  return (
    <div
...
export default function Home() {
  ...
  const { guestName, hasToken, tokenValue } = useMemo(() => {
    if (typeof window === "undefined") {
      return { guestName: "Tamu Undangan", hasToken: false, tokenValue: null as string | null };
    }
    ...
    return {
      guestName: cleaned ? cleaned.slice(0, 60) : "Tamu Undangan",
  ...
  const downloadQrImage = async () => {
    if (!tokenValue) {
      showToast("Token tidak tersedia", "error");
      return;
    }
    setIsExporting(true);
    try {
      ...
      if (!card) {
        showToast("Kartu belum siap", "error");
        return;
      }
      const baseWidth = card.offsetWidth || 420;
      const baseHeight = card.offsetHeight || Math.round(baseWidth * 0.72);
      const pixelRatio = 2;
      ...
      const safeName = (guestName || "guest").replace(/\s+/g, "_");
      a.download = `QR-Card-${safeName}.jpg`;
      a.href = dataUrl;
  ...
  return (
    <>
          ...
          <div className="qr-modal-root" onClick={(e) => e.stopPropagation()} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <div className={`card ${isExporting ? "is-exporting" : ""}`} ref={qrCardRef}>
              <style>{`
                ...
                .qr-modal-root .banner { width: 100%; height: 120px; background: #2a1e0a; position: relative; overflow: hidden; display: flex; align-items: center; justify-content: center; }
                .qr-modal-root .banner-overlay { position: absolute; inset: 0; background: linear-gradient(135deg, #1a1005 0%, #3d2b0d 40%, #5a3e14 70%, #2a1e0a 100%); opacity: 0.88; }
                .qr-modal-root .banner-content { position: relative; z-index: 2; text-align: center; width: 100%; padding: 0 20px; color: #f0e8d4; }
                .qr-modal-root .banner-pretitle { font-family: -apple-system, 'Helvetica Neue', sans-serif; font-size: 7.5px; letter-spacing: 0.3em; color: #c9a96e; text-transform: uppercase; margin-bottom: 8px; }
                ...
                .qr-modal-root .banner-meta-item { font-family: -apple-system, sans-serif; font-size: 8px; letter-spacing: 0.18em; color: #a08050; text-transform: uppercase; }
                .qr-modal-root .banner-meta-dot { width: 2px; height: 2px; background: #6a5030; border-radius: 50%; }
                ...
                .qr-modal-root .info-divider { width: 32px; height: 0.5px; background: #c9a96e; margin-bottom: 12px; }
              ...
              <button className="btn-close" onClick={() => setShowQrModal(false)} title="Tutup">
                <svg viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <line x1="1.5" y1="1.5" x2="8.5" y2="8.5" stroke="#c9a96e" strokeWidth="1.5" strokeLinecap="round" />
                  <line x1="8.5" y1="1.5" x2="1.5" y2="8.5" stroke="#c9a96e" strokeWidth="1.5" strokeLinecap="round" />
                </svg>
              </button>
      ...
      <main>
        ...
        <header className="hero">
          ...
          <div className="reveal reveal-up delay-5" style={{ position: "absolute", bottom: "86px", left: 0, right: 0, display: "flex", justifyContent: "center", zIndex: 30 }}>
            <a
              href="https://www.google.com/calendar/render?action=TEMPLATE&text=The+Wedding+of+Fizah+%26+Hanif&details=Kami+mengundang+Anda+untuk+hadir+di+pernikahan+kami.&location=Gedung+Pancasila,+Kabupaten+Ketapang&dates=20260517T020000Z/20260517T140000Z"
              target="_blank"
          ...
          <div className="wishes-bg">
            <svg width="100%" height="100%" viewBox="0 0 430 600" preserveAspectRatio="xMidYMid slice">
              <g fill="none" stroke="#EDD9A3" strokeWidth="0.5">
                <path d="M0,300 Q215,100 430,300 Q215,500 0,300Z" opacity="0.3" />
        ...
        <section className="wishes-section">
          ...
          <div className="wishes-inner">
            <div className="wishes-heading reveal reveal-up">
              ...
              <p className="wishes-subtitle">Berikan ucapan &amp; doa untuk kedua mempelai</p>
            </div>
      ...
      {isInvitationOpen && (
        ...
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#EDD9A3" strokeWidth="1.5">
